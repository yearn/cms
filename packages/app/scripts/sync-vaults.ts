...
const REGISTRY_HANDLERS = {
  ...
  '0x727fe1759430df13655ddb0731df210a8a1f8b54': {
    label: 'YEARN',
    handler: 'V02_STANDARD',
    tag: '',
  }, // Fantom V2
  '0x1ba4eb0f44ab82541e56669e18972b0d6037dfe0': {
    label: 'YEARN',
    handler: 'V02_STANDARD',
    tag: '',
  }, // Optimism V2
  ...
  '0x50c1a2ea0a861a967d9d0ffe2ae4012c2e053804': {
    label: 'YEARN',
    handler: 'V02_EXPERIMENTAL',
    tag: '',
  }, // Ethereum V2 Experimental
  ...
  '0x79286dd38c9017e5423073ba1d328373a35e4a6c': {
    label: 'YEARN',
    handler: 'V03',
    tag: '',
  }, // Optimism V3
  '0xf3885ede00171997bfadaa98e01e167b53a78ec5': {
    label: 'YEARN',
    handler: 'V03',
    tag: '',
  }, // Base V3
  ...
  '0xff5e3a7c4cbfa9dd361385c24c3a0a4ee63ce500': {
    label: 'YEARN',
    handler: 'V04',
    tag: '',
  }, // Polygon V4
  ...
  '0xd5967178702250d9f0eac34258ebba99b9a28ed0': {
    label: 'YEARN',
    handler: 'V06',
    tag: '',
  }, // Polygon V6 Gamma
  ...
  '0x842b22eb2a1c1c54344eddbe6959f787c2d15844': {
    label: 'COVE',
    handler: 'V05',
    tag: '',
  }, // Cove
  '0x8020fb37b21e0ef1707ada7a914baf44f9045e52': {
    label: 'POOL_TOGETHER',
    handler: 'V05',
    tag: '',
  }, // Pool Together Arbitrum
} as const
...
function categorizeVault(name: string): string {
  ...
  if (bitcoinPatterns.some((btc) => lowerName.includes(btc)) || ethPatterns.some((eth) => lowerName.includes(eth))) {
    return 'Volatile'
  }
...
async function updateVaultsForChain(chainId: number, kongVaults: KongVault[]): Promise<number> {
  ...
  const existingAddresses = new Set(existingVaults.map((vault) => vault.address.toLowerCase()))

  // Find new vaults that don't exist in the current data
  const newVaults = chainVaults.filter((vault) => !existingAddresses.has(vault.address.toLowerCase()))
  ...
  const updatedVaults = [...existingVaults, ...newVaults.map(createVaultFromKong)]

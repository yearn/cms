import type { StrategyMetadata } from '@webops/cms/core'
...
function List() {
  ...
  const filter: StrategyMetadata[] = useMemo(() => {
    return strategies.filter((strategy: StrategyMetadata) => 
  ...
  return <InfiniteScroll 
    ...
    {items.map((strategy: StrategyMetadata) => (
      <Link key={`${strategy.chainId}-${strategy.address}`} 
        ...
        <div>{strategy.address.slice(0, 6)}..{strategy.address.slice(-6)}</div>

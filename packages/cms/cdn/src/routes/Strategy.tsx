...
import { useStrategyMeta } from '@webops/cms/react'
import { chains } from '@webops/core'
import { Suspense } from 'react'
import { PiGitPullRequest } from 'react-icons/pi'
import { useParams } from 'react-router-dom'
import { StrategyMetadataSchema } from '../../../core/types/StrategyMetadata'
...
function PullRequestButton() {
  ...
  const createPullRequest = useMutation({
    mutationFn: async () => {
      ...
      const path = `packages/cms/cdn/content/strategies/${vault.chainId}.json`
...
function Provider({ children }: { children: React.ReactNode }) {
  ...
  const strategy = strategies.find(v => 
    v.chainId.toString() === chainId && 
    v.address.toLowerCase() === address?.toLowerCase()
  )

  if (!strategy) { throw new Error('Stratgy not found') }

  return <MetaDataProvider schema={StrategyMetadataSchema} o={strategy}>
    {children}
  </MetaDataProvider>
}
